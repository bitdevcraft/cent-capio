...
import { MenubarSub, MenubarSubContent } from "@radix-ui/react-menubar";
import { MenubarItem, MenubarSubTrigger } from "../../shadcn/menubar";

export function EmojiMenubarButton({
  options,
  ...props
}: {
  options?: EmojiDropdownMenuOptions;
} & React.ComponentPropsWithoutRef<typeof MenubarSub>) {
  const { emojiPickerState, isOpen, setIsOpen } =
    useEmojiDropdownMenuState(options);

  return (
    <MenubarSub onOpenChange={setIsOpen} defaultOpen={isOpen}>
      <MenubarSubTrigger {...props} className="p-0">
        <MenubarItem>
          <SmileIcon /> Emoji
        </MenubarItem>
      </MenubarSubTrigger>
